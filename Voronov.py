...
def generate_table_dat(count_row: int) -> None:
    ...
    for i in range(count_row):
        text += f"00{i}|{day}|{random.choice(times)}|{random.choice(subjects)}|{random.choice(cab)}|{random.choice(surnames)}\n"
...
def timetable_for_teacher():
    ...
    for record in data:
        record = record.split("|")
        if surname == record[5]:
            result_list_day += record[1]
            result_list_time += record[2]
    label_table["text"] = f"День недели: {''.join(result_list_day)}"
